...
export default async function optAssets(
        ...
        optInTxns.push(
            createTransactionToSign(
                algosdk.makeAssetTransferTxnWithSuggestedParamsFromObject({
                    ...
                    note: new Uint8Array(
                        Buffer.from(
                            ` I am an asset opt-${optPrefix} transaction for algoworld city pack escrow, thank you for using AlgoRealm (☞ ͡° ͜ʖ ͡°)☞`
                        )
                    ),
    ...
    const signedSaveSwapConfigTxns = await creatorWallet
        ...
        .catch(() => {
            dispatch(
                setLoadingIndicator({ isLoading: false, message: undefined })
            );
            return undefined;
        });
